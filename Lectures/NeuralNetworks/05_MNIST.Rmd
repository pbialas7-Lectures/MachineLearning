...
idx = np.random.choice(len(train_labels),8)
for i in range(8):
    ax=axes.ravel()[i]
    ax.imshow(train_features[idx[i]].numpy(), cmap='Greys');
    ax.set_title(train_labels[idx[i]].item(), fontsize=20)
```
...
```{python slideshow={'slide_type': 'fragment'}, editable=TRUE}
mist_test_accuracy=model_accuracy(model, test_dataset)
mist_test_accuracy
```

<!-- #region {"editable": true, "slideshow": {"slide_type": "slide"}} -->
**Problem** What would be the accuracy on five digits zip-code?
<!-- #endregion -->

<!-- #region {"editable": true, "slideshow": {"slide_type": "skip"}} -->
Assuming that the probabilities of getting  digits right are independent (is this true?) the probability of getting each digit right is 
<!-- #endregion -->

```{python editable=TRUE, slideshow={'slide_type': 'skip'}}
mist_test_accuracy**5
```
...
```{python slideshow={'slide_type': 'fragment'}, editable=TRUE}
model_copy = torch.load("mnist.pt", weights_only=False)
```
...
with torch.no_grad():
    ...
    ac = torch.sum(
      torch.argmax(pred,1)==train_dataset[:][1]).to(dtype=torch.float32)/len(train_dataset)
ac 
```
...
```{python slideshow={'slide_type': 'fragment'}, editable=TRUE}
model = torch.nn.Sequential(
    nn.Linear(28*28,1200), nn.Sigmoid(),
    nn.Linear(1200,600), nn.Sigmoid(),
    nn.Linear(600,300), nn.Sigmoid(),
    nn.Linear(300,10)
)
```
...
for e in range(100):
    for features, labels in train_loader:        
        optim.zero_grad()
        pred = model(features.to(torch_device))
        loss = ce_loss(pred,labels.to(torch_device))
        loss.backward()
        optim.step()   
    if (e+1)%10 == 0:  
      print(e, loss.item())        
...
```{python editable=TRUE, slideshow={'slide_type': 'fragment'}}
proba = torch.nn.functional.softmax(y,1)
```

```{python editable=TRUE, slideshow={'slide_type': 'fragment'}}
wrong_predictions, = torch.where((torch.argmax(proba,1)==labels)==False)
```
...
def plot_res(index):
  ...
  ax[0].stem(np.arange(0,10), proba[index], basefmt='blue');
  ax[0].axvline(true_label, color='orange');
  ax[1].imshow(features[index].numpy().reshape(28,28), cmap='Greys');
  return fig, ax  
```

```{python editable=TRUE, slideshow={'slide_type': 'slide'}}
plot_res(wrong_predictions[0]);
```

```{python editable=TRUE, slideshow={'slide_type': 'slide'}}
plot_res(wrong_predictions[1]);
```
