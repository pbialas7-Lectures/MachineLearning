...
```{python slideshow={'slide_type': 'slide'}, tags=c("answer"), editable=TRUE, jupyter={'source_hidden': True}}
...
```{python tags=c("answer"), editable=TRUE, slideshow={'slide_type': ''}, jupyter={'source_hidden': True}}
hs = np.linspace(120,215,400)
...
This can be viewed as the probability that given a  positive results that examplar is really a positive. 
<!-- #endregion -->
...
To answer this question you have to  assign a cost to each type of errors. Let's assume that you consider the cost of your teeth as five times bigger then cost of being rejected. Then the expected cost is: 
<!-- #endregion -->
...
<!-- #region {"slideshow": {"slide_type": "notes"}, "editable": true} -->
Now the treshold is considerably higher. That's logical as mistaking men for women (false positive) is more costly, you want to be more sure that this is a man before  asking this person out on   date. 
