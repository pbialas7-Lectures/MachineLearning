...
ax.plot(xs, (1-pi)*pdf[:,0]+pi*pdf[:,1],label = '$\pi \cdot  p(x|\mu_1,\sigma_1) +  (1-\pi) \cdot p(x|\mu_0,\sigma_0)$')
ax.plot(xs, (1-pi)*pdf[:,0], label = '$(1-\pi) \cdot  p(x|\mu_0,\sigma_0)$');
ax.plot(xs, pi*pdf[:,1], label = '$\pi \cdot  p(x|\mu_1,\sigma_1)$');
ax.legend(loc = 2);
plt.close()
```
...
```{python slideshow={'slide_type': 'skip'}}
fig, ax = plt.subplots()
ax.scatter(range(len(thetas)), thetas[:,0], s=10, alpha=0.5, label='$\pi$');
ax.axhline(0.3);
ax.axhline(0.7);
ax.legend();
plt.close()
...
```{python slideshow={'slide_type': 'skip'}}
fig, ax = plt.subplots()
ax.scatter(range(len(thetas)), thetas[:,1], s=10, alpha=0.5, label = "$\mu_1$", color = 'blue');
ax.axhline(mus[0], color = 'blue')
ax.scatter(range(len(thetas)), thetas[:,3], s=10, alpha=0.5, label = "$\mu_2$", color = 'orange');
...
```{python slideshow={'slide_type': 'skip'}}
fig, ax = plt.subplots()
ax.scatter(range(len(thetas)), thetas[:,2], s=10, alpha=0.5, label = "$\sigma_1$", color = 'blue');
ax.axhline(stds[0], color = 'blue')
ax.scatter(range(len(thetas)), thetas[:,4], s=10, alpha=0.5, label = "$\sigma_2$", color = 'orange');
...
for train_i, test_i in folder.split(hc_train, hc_lbl_train):
    f1+=evaluate(5,5,hc_train[train_i], hc_lbl_train[train_i], hc_train[test_i], hc_lbl_train[test_i])
